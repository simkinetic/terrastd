...
terralib.linklibrary("src/std@v0/sleef/build-sleef/lib/libsleef" .. ext)
...
for T, _ in pairs(vecmath) do
    for name, precision in pairs(func) do
        ...
        for _, width in ipairs(supported_width) do
            ...
            vecmath[T][name][width] = terralib.externfunction("Sleef_" .. vecname, vector(T, len) -> vector(T, len))
        end
    end
end
